...
def test_monthly_da_scheduler_comparison():
    ...
    for i in range(num_iterations):
        ...
        da_fc_reg, rt_fc_reg = get_forecasts_for_da(
            data, current_time=current_time, horizon_hours=24 * AMT_DAYS, method="regression", verbose=False
        )
        da_fc_pers, rt_fc_pers = get_forecasts_for_da(
            data, current_time=current_time, horizon_hours=24 * AMT_DAYS, method="persistence", verbose=False
        )
        da_real, rt_real = get_forecasts_for_da(
            data, current_time=current_time, horizon_hours=24 * AMT_DAYS, method="perfect", verbose=False
        )
        ...
        for name, params in scenarios_config.items():
            ...
            # Revenue Calc
            revenue_chunk = result.da_energy_bids * da_real + result.rt_energy_bids * rt_real
    ...
    assert len(results_comparison) == len(scenarios_config), "Not all scenarios generated results"
    for name, res in results_comparison.items():
        assert not res["cumulative_revenue_real"].empty, f"{name} produced empty results"
    ...
    sorted_names = sorted(results_comparison.keys(), key=lambda k: results_comparison[k]['total_revenue'], reverse=True)
    ...
    ax.set_title(f"Cumulative Real Revenue over {TOTAL_DAYS} Days\n(2-Day Rolling Optimization)")
