...
def test_da_scheduler():
    ...
    da_prices_forecast_persistence, rt_prices_forecast_persistence = (
        get_forecasts_for_da(
            data,
            current_time=current_time,
            horizon_hours=24 * AMT_DAYS,
            method="persistence",
    ...
    sorted_names = sorted(
        results_comparison.keys(), key=lambda k: 1 if "Perfect Prediction" in k else 0
    )
