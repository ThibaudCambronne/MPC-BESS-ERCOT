...
def run_simulation_for_date(data, date_str):
    ...
    da_prices, rt_prices_s1, rt_prices_sim = setup_simulation_data(
        data, date_str, sim_hours, mpc_horizon
    )
    ...
    for t in range(sim_steps):
        ...
        p_req = (
            res.power_setpoint
            if res.solve_status in ["optimal", "max_iter", "optimal_with_slack"]
            else 0.0
        )
        ...
        # Update State
        if p_green >= 0:
            e_next = e_curr + p_green * battery.efficiency_charge * DELTA_T
        else:
            e_next = e_curr + p_green / battery.efficiency_discharge * DELTA_T
...
def plot_detailed_simulation(
    date_str, steps, revenue, prices, soc, p_green, p_blue, p_orange
):
