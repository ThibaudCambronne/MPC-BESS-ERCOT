...
def setup_simulation_data(sim_date, sim_horizon_hours, mpc_horizon_hours):
    ...
    da_prices_stage1 = get_forecast(
        data, current_time=da_run_time, horizon_hours=14 + 24, market="DA", method="perfect", verbose=False
    )[-96:] 

    rt_prices_stage1 = get_forecast(
        data, current_time=da_run_time, horizon_hours=14 + 24, market="RT", method="perfect", verbose=False
    )[-96:]

    # 28h Forecast for Stage 2 (Sim + Buffer)
    total_sim_hours = sim_horizon_hours + mpc_horizon_hours
    rt_prices_sim = get_forecast(
        data, current_time=sim_date, horizon_hours=total_sim_hours, market="RT", method="perfect", verbose=False
    )
...
def run_simulation_comparison():
    ...
    try:
        da_prices_s1, rt_prices_s1, rt_prices_sim = setup_simulation_data(sim_date, sim_hours, mpc_horizon_hours)
    ...
    da_revenue_total = np.sum(-(da_prices_s1.values * da_result.da_energy_bids * DELTA_T))
    print(f"Stage 1 DA Revenue (Locked In): ${da_revenue_total:.2f}")

    # --- Baseline Strategy ---
    rt_prices_baseline = rt_prices_sim.iloc[:sim_steps]
    rev_baseline_step, soc_baseline = run_heuristic_strategy(rt_prices_baseline, battery, sim_steps)
    ...
    for t in range(sim_steps):
        ...
        if t % 24 == 0: print(f"  Simulating Step {t}/{sim_steps}...")
        ...
        power_setpoint = result.power_setpoint if result.solve_status in ["optimal", "max_iter", "optimal_with_slack"] else 0.0

        # Physics
        if power_setpoint >= 0:
            e_next = current_soc * battery.capacity_mwh + (power_setpoint * battery.efficiency_charge) * DELTA_T
        else:
            e_next = current_soc * battery.capacity_mwh + (power_setpoint / battery.efficiency_discharge) * DELTA_T
...
def plot_results(sim_steps, cum_rev_mpc, cum_rev_baseline, prices, soc_mpc, soc_baseline, power_mpc, da_bids):
    try:
        ...
        ax1.fill_between(times, cum_rev_mpc, cum_rev_baseline, where=(cum_rev_mpc > cum_rev_baseline), interpolate=True, color="green", alpha=0.1)
        ax1.legend()
        ax1.grid(True)
