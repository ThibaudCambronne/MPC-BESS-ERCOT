...
from src.utils import DAScheduleResult

def generate_synthetic_data(start_date, steps_needed):
    """Generates synthetic price and DA schedule data."""
    hours_duration = steps_needed * 0.25
    t = np.linspace(0, hours_duration, steps_needed)
    
    # 1. Synthetic Day-Ahead Price ($/MWh)
    # Pattern: Base 30, Peak at 19:00, Low at 04:00
    t_mod = t % 24
    da_price = 30 + 40 * np.exp(-0.1 * (t_mod - 19)**2) - 20 * np.exp(-0.1 * (t_mod - 4)**2)
    
    # 2. Synthetic Real-Time Price ($/MWh)
    noise = np.random.normal(0, 5, steps_needed)
    rt_price = da_price.copy() + noise
    
    # Add random spikes (volatility)
    spike_indices = np.random.choice(range(steps_needed), size=max(1, int(steps_needed/30)), replace=False)
    rt_price[spike_indices] += np.random.uniform(50, 200, len(spike_indices))
    
    # 3. Synthetic DA Schedule (MW) - Simple Arbitrage
    da_bids = np.zeros(steps_needed)
    charge_mask = da_price < 25
    discharge_mask = da_price > 50
    da_bids[charge_mask] = 5.0
    da_bids[discharge_mask] = -5.0
    
    idx = pd.date_range(start=start_date, periods=steps_needed, freq="15min")
    rt_series = pd.Series(data=rt_price, index=idx)
    
    return rt_series, da_bids
...
def run_heuristic_strategy(rt_prices, battery, steps):
    ...
    for t in range(steps):
        ...
        if p_cmd > 0: # Charge
            energy_to_add = p_cmd * battery.efficiency_charge * dt_h
            if current_soc * battery.capacity_mwh + energy_to_add > battery.soc_max * battery.capacity_mwh:
                p_cmd = 0 # Simple cut-off
        elif p_cmd < 0: # Discharge
            energy_to_remove = abs(p_cmd) / battery.efficiency_discharge * dt_h
            if current_soc * battery.capacity_mwh - energy_to_remove < battery.soc_min * battery.capacity_mwh:
                p_cmd = 0 # Simple cut-off
                
        # Dynamics
        if p_cmd >= 0:
            e_next = current_soc * battery.capacity_mwh + (p_cmd * battery.efficiency_charge) * dt_h
        else:
            e_next = current_soc * battery.capacity_mwh + (p_cmd / battery.efficiency_discharge) * dt_h
            
        current_soc = e_next / battery.capacity_mwh
        current_soc = np.clip(current_soc, 0.0, 1.0)
        
        step_revenue = -(price * p_cmd * dt_h)
...
def run_simulation_comparison():
    ...
    rev_baseline_step, soc_baseline = run_heuristic_strategy(rt_forecast_full, battery, sim_steps)
    ...
    for t in range(sim_steps):
        if t % 24 == 0: print(f"MPC Step {t}/{sim_steps}...")
        ...
        power_setpoint = result.power_setpoint if result.solve_status in ['optimal', 'max_iter'] else 0.0
        
        # Plant Dynamics
        dt_h = 0.25
        if power_setpoint >= 0:
            e_next = current_soc * battery.capacity_mwh + (power_setpoint * battery.efficiency_charge) * dt_h
        else:
            e_next = current_soc * battery.capacity_mwh + (power_setpoint / battery.efficiency_discharge) * dt_h
            
        current_soc = np.clip(e_next / battery.capacity_mwh, 0.0, 1.0)
        
        # Revenue
        step_price = rt_forecast_full.iloc[t]
        rev_mpc_step.append(-(step_price * power_setpoint * dt_h))
    ...
    try:
        ...
        ax1.plot(times, cum_rev_mpc, 'g-', linewidth=2, label=f'MPC (Total: ${cum_rev_mpc[-1]:.2f})')
        ax1.plot(times, cum_rev_baseline, 'k--', linewidth=1.5, label=f'Heuristic (Total: ${cum_rev_baseline[-1]:.2f})')
        
        # Fill area to show benefit
        ax1.fill_between(times, cum_rev_mpc, cum_rev_baseline, where=(cum_rev_mpc > cum_rev_baseline), 
                         interpolate=True, color='green', alpha=0.1, label='Added Value')
